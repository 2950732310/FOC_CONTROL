...
static const ParamDef_t g_param_table[] =
{
    ...
    { "flash_control",  PARAM_TYPE_INT,  &command_flag.flash_control,       "",    "[R/W]1:Flash写入" },
    { "state_mode",      PARAM_TYPE_INT,    &motor.State_Mode,              "",    "[R/W]运行状态"      },
    { "vq_set",         PARAM_TYPE_FLOAT,  &motor.foc.vq_set,               "V",   "[R/W]开环电压设置"       },
    { "i_q_ref",         PARAM_TYPE_FLOAT,  &motor.foc.i_q_ref,               "A",   "[R/W] Q轴电流参考"      },
    { "i_d_ref",         PARAM_TYPE_FLOAT,  &motor.foc.i_d_ref,               "A",   "[R/W] D轴电流参考"      },
    { "vel_ref",         PARAM_TYPE_FLOAT,  &motor.foc.vel_ref,               "rad/s",   "[R/W] 速度参考"      },
    { "id_kp",           PARAM_TYPE_FLOAT,  &motor.foc.flash_data.id_kp,     "",   "[R/W] D轴电流环比例系数" },
    { "id_ki",           PARAM_TYPE_FLOAT,  &motor.foc.flash_data.id_ki,     "",   "[R/W] D轴电流环积分系数" },
    { "iq_kp",           PARAM_TYPE_FLOAT,  &motor.foc.flash_data.iq_kp,     "",   "[R/W] Q轴电流环比例系数" },
    { "iq_ki",           PARAM_TYPE_FLOAT,  &motor.foc.flash_data.iq_ki,     "",   "[R/W] Q轴电流环积分系数" },
    { "vel_kp",          PARAM_TYPE_FLOAT,  &motor.foc.flash_data.vel_kp,    "",   "[R/W] 速度环比例系数" },
    { "vel_ki",          PARAM_TYPE_FLOAT,  &motor.foc.flash_data.vel_ki,    "",   "[R/W] 速度环积分系数" },

    /* ---- 只读参数 ---- */
    { "angle",          PARAM_TYPE_FLOAT,  &encoder_data.mec_angle,         "",   "[R] 机械角度(弧度)" },
    { "angle_deg",      PARAM_TYPE_FLOAT,  &encoder_data.mec_angle_deg,     "",   "[R] 机械角度(角度)" },
    { "elec_angle",          PARAM_TYPE_FLOAT,  &encoder_data.elec_angle,   "",   "[R] 电角度(rad)" },
    { "pos_estimate",    PARAM_TYPE_FLOAT,  &encoder_data.pos_estimate_,    "",   "[R] 圈数" },
    { "vel_estimate",    PARAM_TYPE_FLOAT,  &encoder_data.vel_estimate_,    "",   "[R] 速度(rad/s)" },
    { "i_d",             PARAM_TYPE_FLOAT,  &motor.foc.i_d,                 "A",  "[R] D轴电流"      },
    { "i_q",             PARAM_TYPE_FLOAT,  &motor.foc.i_q,                 "A",  "[R] Q轴电流"      },
    { "i_a",             PARAM_TYPE_FLOAT,  &motor.foc.i_a,                 "A",  "[R] A相电流"      },
    { "i_b",            PARAM_TYPE_FLOAT,  &motor.foc.i_b,                 "A",  "[R] B相电流"      },
    { "i_c",            PARAM_TYPE_FLOAT,  &motor.foc.i_c,                 "A",  "[R] C相电流"      },
    { "v_bus",          PARAM_TYPE_FLOAT,  &motor.foc.vbus,                "V",  "[R] 母线电压"      },
    { "ia_zero",        PARAM_TYPE_FLOAT,  &motor.foc.flash_data.ia_zero,  "",   "[R] A相电流偏移"      },
    { "ib_zero",        PARAM_TYPE_FLOAT,  &motor.foc.flash_data.ib_zero,  "",   "[R] B相电流偏移"      },
};
