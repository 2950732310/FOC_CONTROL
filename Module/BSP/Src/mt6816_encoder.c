...
ENCODER_DATA encoder_data = {
    ...
    .CS_Port              = GPIOC,                  // 片选引脚的端口
    .CS_Pin               = GPIO_PIN_11,          // 片选引脚的引脚号
    .pole_pairs           = POLE_PAIRS,           // 设置极对数为电机参数
    .encoder_offset       = 0,                    // 设置电角度偏移量
    .dir                  = CW,                   // 设置编码器旋转方向为顺时针
    .raw                  = 0,                    // 初始化原始计数值为0
    .cnt                  = 0,                    // 计数器
...
void GetMotor_Angle(ENCODER_DATA *encoder)
{
    ...
    encoder->mec_angle_deg = encoder->mec_angle * 180.0f / M_PI; // 机械角度(角度)
}
