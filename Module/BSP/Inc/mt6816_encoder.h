...
#define MT6816_SPI_CS_L() HAL_GPIO_WritePin(GPIOC, GPIO_PIN_11, GPIO_PIN_RESET)
#define MT6816_SPI_CS_H() HAL_GPIO_WritePin(GPIOC, GPIO_PIN_11, GPIO_PIN_SET)
#endif
...
typedef struct
{
    ...
    float mec_angle;             // 机械角度(弧度)
    float mec_angle_deg;         // 机械角度(角度)
    float elec_angle;            // 电角度，对应于机械角度的电机转子位置（根据电机的极对数计算得到）
    float encoder_offset;        // 编码器偏移量(弧度)
    float speed;                 // 速度(rad/s)
    float last_angle;            // 上一次的角度(弧度)
